...
def convert_to_unicode(text):
    ...
    if six.PY3:
        ...
        raise ValueError("No running on Python 2 or Python 3.")
...
def whitespace_tokenize(text):
    """Running white space cleaning for text"""
    text = text.strip()
    if not text:
        return []
    tokens = text.split()
...
class FullTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        for token in self.basic_tokenizer.tokenize(text):
            for subtoken in self.wordpiece_tokenizer.tokenize(token):
    ...
    def tokenize(self, text):
        ...
        for token in orig_tokens:
            if self.do_lower_case:
                token = token.lower()
                token = self._run_strip_accent(token)
